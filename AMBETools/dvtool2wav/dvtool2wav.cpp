...
int main(int argc, char** argv)
{
	...
	if (!ret) {
		delete reader;
		::wxUninitialize();
		::fprintf(stderr, "dvtool2wav: unable to open the input file - %s\n", argv[1U]);
		return 1;
	}
	...
	if (!ret) {
		...
		delete writer;
		::wxUninitialize();
		::fprintf(stderr, "dvtool2wav: unable to open the output file - %s\n", argv[2U]);
		return 1;
	}
	...
	switch (type) {
		...
		default:
			...
			delete writer;
			::wxUninitialize();
			::fprintf(stderr, "dvtool2wav: invalid Dongle type specified\n");
			return 1;
	}

	dvtool2wav program(dongle, reader, writer);
	...
	if (!ret) {
		...
		delete dongle;
		::wxUninitialize();
		::fprintf(stderr, "dvtool2wav: couldn't find the Dongle on the port specified\n");
		return 1;
	}
...
dvtool2wav::dvtool2wav(CDongleThread* dongle, CDVTOOLFileReader* reader, CWAVFileWriter* writer) :
m_dongle(dongle),
m_reader(reader),
m_writer(writer)
{
...
dvtool2wav::~dvtool2wav()
{
}

void dvtool2wav::decodeCallback(const wxFloat32* audio, unsigned int length)
{
	if (m_writer == NULL)
		return;
