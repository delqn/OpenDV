...
CDCSGatewayEchoUnit::CDCSGatewayEchoUnit(IDCSGatewayCallback* handler, const wxString& callsign) :
m_handler(handler),
m_callsign(callsign),
m_status(ES_IDLE),
...
CDCSGatewayEchoUnit::~CDCSGatewayEchoUnit()
{
	delete[] m_data;
}

void CDCSGatewayEchoUnit::writeHeader(const CDCSGatewayHeaderData& header)
{
	if (m_status != ES_IDLE)
		return;

	m_header = new CDCSGatewayHeaderData(header);

	m_in     = 0U;		
	m_status = ES_RECEIVE;
}

void CDCSGatewayEchoUnit::writeData(const CDCSGatewayAMBEData& data)
{
	if (m_status != ES_RECEIVE)
		return;

	if (m_in < MAX_FRAMES) {
		m_data[m_in] = new CDCSGatewayAMBEData(data);
		m_in++;
	}
...
void CDCSGatewayEchoUnit::clock(unsigned int ms)
{
	m_timer.clock(ms);
...
void CDCSGatewayEchoUnit::cancel()
{
	for (unsigned int i = 0U; i < MAX_FRAMES; i++) {
		if (m_data[i] != NULL) {
			delete m_data[i];
