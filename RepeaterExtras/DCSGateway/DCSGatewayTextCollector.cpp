...
CDCSGatewayTextCollector::CDCSGatewayTextCollector() :
m_data(NULL),
m_buffer(NULL),
m_slowData(SS_FIRST),
m_has0(false),
...
m_has3(false)
{
	m_data   = new char[SLOW_DATA_TEXT_LENGTH];
...
void CDCSGatewayTextCollector::writeData(const CDCSGatewayAMBEData& data)
{
	if (data.isSync()) {
		sync();
		return;
...
void CDCSGatewayTextCollector::reset()
{
	m_slowData = SS_FIRST;
	m_has0 = false;
	m_has1 = false;
...
void CDCSGatewayTextCollector::sync()
{
	m_slowData = SS_FIRST;
}

bool CDCSGatewayTextCollector::hasData() const
{
	return m_has0 && m_has1 && m_has2 && m_has3;
}

wxString CDCSGatewayTextCollector::getData()
{
	wxString text;
	...
	return wxString(m_data, wxConvLocal, SLOW_DATA_TEXT_LENGTH);
}
