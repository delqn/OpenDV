...
int main(int argc, char** argv)
{
	...
	if (!res) {
		::fprintf(stderr, "parrotcontrollerd: failed to initialise the wxWidgets library, exiting\n");
		return -1;
	}
	...
	if (daemon) {
		...
		if (pid < 0) {
			::fprintf(stderr, "parrotcontrollerd: error in fork(), exiting\n");
			::wxUninitialize();
			return 1;
		}
	...
	CParrotControllerD repeater(nolog, logDir, confDir, name);
...
CParrotControllerD::CParrotControllerD(bool nolog, const wxString& logDir, const wxString& confDir, const wxString& name) :
m_name(name),
m_nolog(nolog),
m_logDir(logDir),
m_confDir(confDir),
...
CParrotControllerD::~CParrotControllerD()
{
}

bool CParrotControllerD::init()
{
	if (!m_nolog) {
...
bool CParrotControllerD::createThread()
{
	CParrotControllerConfig config(m_confDir, m_name);
	...
	bool keepFile;
	config.getMode(mode, beaconTime, beaconFileName, turnaroundTime, keepFile);
	...
	config.getNetwork(repeaterAddress, repeaterPort, localAddress, localPort);
