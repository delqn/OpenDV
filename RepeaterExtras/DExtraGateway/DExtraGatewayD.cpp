...
int main(int argc, char** argv)
{
	...
	if (!res) {
		::fprintf(stderr, "dextragatewayd: failed to initialise the wxWidgets library, exiting\n");
		return -1;
	}
	...
	if (daemon) {
		...
		if (pid < 0) {
			::fprintf(stderr, "dextragatewayd: error in fork(), exiting\n");
			::wxUninitialize();
			return 1;
		}
	...
	CDExtraGatewayD gateway(nolog, logDir, confDir, name);
...
CDExtraGatewayD::CDExtraGatewayD(bool nolog, const wxString& logDir, const wxString& confDir, const wxString& name) :
m_name(name),
m_nolog(nolog),
m_logDir(logDir),
m_confDir(confDir),
...
CDExtraGatewayD::~CDExtraGatewayD()
{
}

bool CDExtraGatewayD::init()
{
	if (!m_nolog) {
...
bool CDExtraGatewayD::createThread()
{
	CDExtraGatewayConfig config(m_confDir, m_name);
